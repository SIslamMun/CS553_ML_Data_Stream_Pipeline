...
from pyflink.datastream.connectors import RabbitMQSource, RMQConnectionConfig
...
def run(source,    weights='old_model/person_m_fusion_openvino_model',  # model path or triton URL
    ...
    model = MultiModel(weights, device=device, dnn=dnn, data=data, fp16=half)
...
def flink_streaming_engine(input_path):
    ...
    rabbitmq_source = RabbitMQSource(rabbitmq_config, "queue_name", True, None)
